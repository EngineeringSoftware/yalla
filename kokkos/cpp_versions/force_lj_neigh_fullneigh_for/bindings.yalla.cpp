...
void run_fullneigh_for(
  ...
  pk_functor_fullneigh_for pk_f(
      &pk_d_rnd_lj1, &pk_d_rnd_lj2, &pk_d_rnd_cutsq, &pk_d_num_neighs_view,
      &pk_d_neighs_view, &pk_d_x, &pk_d_f, &pk_d_type);
  Kokkos::parallel_for(
      pk_kernel_name,
      Kokkos::RangePolicy<pk_exec_space, pk_functor_fullneigh_for::fullneigh_for_tag>(
...
void wrapper_fullneigh_for(pybind11::kwargs kwargs) {
  run_fullneigh_for(
      kwargs["rnd_lj1"]
...
PYBIND11_MODULE(
    PLACEHOLDER, k) {
  k.def("wrapper_fullneigh_for", &wrapper_fullneigh_for);
}